...
lists = []
for l in list:
    if (os.path.splitext(l)[1] == '.air') and l != 'common.air' and l != 'header.air':
        lists.append(l)
    ...
    for dev in devices:
        if (not run_all and results['tests'].get(dev) and
           results['tests'].get(dev).get('status') == 0):
            ...
            continue
        log_dir = get_log_dir(dev+'_'+air, air)
        cmd = [
            "airtest",
            "run",
             air,
            "--device",
            "Android:///" + dev+"?touch_method=ADBTOUCH",
            "--log",
            log_dir
        ]
        try:
...
if __name__ == '__main__':
    ...
    # 重新运行所有脚本
    for air in airs:
        data2 = run(devices, air, run_all=True) 
        print(data2)
        for key in (data1.keys() | data2.keys()):
            if key == "script":
                data1.setdefault(key,[]).extend(data2[key])
            elif key == "tests":
                data1.setdefault(key, {}).update(data2[key])
